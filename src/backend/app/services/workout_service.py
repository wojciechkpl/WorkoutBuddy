...
from app.models import Exercise, Workout, WorkoutExercise, User, UserStats
...
class WorkoutService:
    ...
    def calculate_workout_metrics(self, workout: Workout):
        ...
        for we in workout.exercises:
            ...
            if exercise.mets and we.duration:
                ...
                calories = exercise.mets * (workout.user.weight or 70) * (we.duration / 60)
    ...
    def update_user_stats(self, user: User, workout: Workout):
        ...
        stats = self.db.query(UserStats).filter(
            ...
            func.date(UserStats.date) == today
        ).first()
    ...
    def get_user_stats(self, user: User, days: int) -> Dict:
        ...
        exercise_counts = self.db.query(
            Exercise.name,
            Exercise.primary_muscle,
            func.count(WorkoutExercise.id).label('count')
        ).select_from(WorkoutExercise).join(
            Exercise
        ).join(
            Workout
        ).filter(
            ...
            func.count(WorkoutExercise.id).desc()
        ).limit(5).all()
        ...
        muscle_distribution = self.db.query(
            Exercise.primary_muscle,
            func.count(WorkoutExercise.id).label('count')
        ).select_from(WorkoutExercise).join(
            Exercise
        ...
        muscle_group_distribution = {
            muscle.value: count
            for muscle, count in muscle_distribution
        }
        ...
        with open(csv_path, 'r', encoding='utf-8') as file:
            ...
            for row in csv_reader:
                exercise = Exercise(
                    ...
                    is_distance_based=row.get('is_distance_based', '').lower() == 'true',
