...
def test_get_exercise_recommendations_success(client, db_session):
    """Test successful exercise recommendations"""
    # Mock user data
    user_data = Mock()
    user_data.id = 1
    ...
    with patch('app.api.exercise_recommender') as mock_recommender:
        ...
        response = client.post("/recommendations/exercises?user_id=1&n_recommendations=5")
...
def test_get_exercise_recommendations_user_not_found(client, db_session):
...
def test_get_exercise_recommendations_ml_service_not_ready(client, db_session):
...
def test_get_similar_users_success(client, db_session):
...
def test_get_similar_users_user_not_found(client, db_session):
...
def test_get_similar_users_ml_service_not_ready(client, db_session):
...
def test_train_models_success(client):
    ...
    # Mock ML models
    with patch('app.api.user_similarity_model') as mock_similarity, \
